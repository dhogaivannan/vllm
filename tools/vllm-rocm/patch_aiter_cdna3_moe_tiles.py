...
def _cdna3_get_kernel_config(m, n, k, routing_data):
    ...
    if cfg["block_m"] >= 64:
        ...
        # Clamp to the 64 KiB LDS: stage A+B tiles must fit. Cascade num_stages
        # then block_n then block_k, mirroring the arch_info accounting.
        def _lds(c):
            return (
                c["block_m"] * c["block_k"] * 2 + c["block_n"] * c["block_k"] // 2
            ) * c["num_stages"]
        while _lds(cfg) > 65536 and cfg["num_stages"] > 1:
            cfg["num_stages"] -= 1
        while _lds(cfg) > 65536 and cfg["block_n"] > 64:
            cfg["block_n"] //= 2
        while _lds(cfg) > 65536 and cfg["block_k"] > 64:
            cfg["block_k"] //= 2
    return cfg
